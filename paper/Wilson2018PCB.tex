...
\caption{\emph{Fitness
    increases gradually towards the fitness peaks.} The average
  fitness of genomes with non-zero fitness decreases linearly with
  each mutation of the genome $m$. Thus, once evolution has entered
  the vicinity of a fitness peak by un-directed search, a constant
  gradient is available to guide evolution directly to the maximally
  fit configuration of the genome. A good fit to the function $h(m,k)$, plotted
  as solid curves, suggests that the smoothness of the
  fitness landscape reflects that genomes with non-zero fitness
  comprise a certain set of boolean interactions that are specified by
  a constant a constant number of binary variables ($k\approx 10$,
  $k\approx 17$ and $k\approx 25$ for $n=4$, $n=5$ and $n=6$, respectively).}
