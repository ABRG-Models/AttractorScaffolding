...
if recompute:
    ...
    m4ff4 = readDataset (genes, m4, '_ff4')
    m5 = np.zeros([1,8])
    genes = 5
    m5 = readDataset (genes, m5, '_ff1')
    m5ff4 = readDataset (genes, m5, '_ff4')
    m6 = np.zeros([1,8])
    genes = 6
    m6 = readDataset (genes, m6, '_ff1')
    m6ff4 = readDataset (genes, m6, '_ff4')
    # Compute bootstraps
    ...
    np.savetxt('m4_ff4_final.txt', m4_ff4_final, fmt='%f')
...
plt.errorbar (m4_ff4_final[:,0]/32, m4ff4y, yerr=m4_ff4_final[:,2], fmt='.',  marker='v', markersize=ms1, linestyle='-', linewidth=lw1, color=c.mediumpurple1)
plt.errorbar (m5_ff4_final[:,0]/80, m5ff4y, yerr=m5_ff4_final[:,2], fmt='.',  marker='v', markersize=ms1, linestyle='-', linewidth=lw1, color=c.darkorchid2)
...
plt.legend(('4 genes, ff1','5 genes, ff1','6 genes, ff1','4 genes, ff4','5 genes, ff4','6 genes, ff4'),frameon=False)

if logplot:
    plt.savefig('smoothfitness_log_ff4.png')
else:
    plt.savefig('smoothfitness_ff4.png')
